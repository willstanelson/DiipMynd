...
export default function ConnectionStatus({ state, retryCount }: ConnectionStatusProps) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
      {/* Animated dot */}
      ...
      {/* Label */}
      <span className="text-xs font-semibold text-slate-700 dark:text-slate-355 dark:text-slate-300 tracking-wide">
        {config.label}
        {state === "reconnecting" && retryCount !== undefined && (
          <span className="text-slate-400 dark:text-slate-500 ml-1">(attempt {retryCount})</span>
        )}
      </span>
    </div>
  );
